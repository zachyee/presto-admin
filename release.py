...
class release(Command):
    ...
    def get_latest_release():
        headers, contents = release.send_get_request(REPOSITORY_API + '/releases/latest')
        return json.loads(contents)

    @staticmethod
    def get_remote_branches():
        headers, contents = release.send_get_request(REPOSITORY_API + '/branches')
        return json.loads(contents)
    ...
    def get_and_check_branch():
        ...
        """
        repo = release.get_and_check_repo()
        current_local_branch = release.get_current_branch(repo)
        release.check_branch_remote_exists(current_local_branch)
        return current_local_branch

    @staticmethod
    def get_last_commit(branch):
        headers, contents = release.send_get_request(REPOSITORY_API + '/commits/' + branch)
        return json.loads(contents)
    ...
    def user_input_username():
        return raw_input('Please input your Github username: ')
    ...
    def get_password(username):
        return getpass.getpass("Enter password for '%s': " % username)

    @staticmethod
    def successful_release(status):
        if status == 201:
            return True
        else:
            return False
    ...
    @staticmethod
    def get_authorization_string(username):
        password = release.get_password(username)
        return base64.standard_b64encode('%s:%s' % (username, password))

    @staticmethod
    def send_post_request(url, data, authorization_string, content_type):
        response = None
        try:
            ...
            headers = response.info()
            contents = response.read()
            if not release.successful_release(status):
                print headers
                print contents
    ...
    @staticmethod
    def send_json_post_request(url, json_data, authorization_string):
        release.send_post_request(url, json_data, authorization_string, 'application/json')

    @staticmethod
    def send_bztar_post_request(url, bztar_data, authorization_string):
        release.send_post_request(url, bztar_data, authorization_string, 'application/x-bzip2')

    @staticmethod
    def send_github_release_posts(json_data, username):
        authorization_string = release.get_authorization_string(username)
        release.send_json_post_request(REPOSITORY_API + '/releases',
        ...
        online_install_path = os.path.join(DIST_DIRECTORY, online_install_name)
        offline_install_path = os.path.join(DIST_DIRECTORY, offline_install_name)
        subprocess.check_call(['make', 'dist-online'])
        with open(online_install_path, mode='rb') as online_installer:
            release.send_bztar_post_request('%s?name=%s' % (release_url, online_install_name),
                                            online_installer.read().decode('cp437'),
                                            authorization_string)
        subprocess.check_call(['make', 'dist-offline'])
        with open(offline_install_path, mode='rb') as offline_installer:
            release.send_bztar_post_request('%s?name=%s' % (release_url, online_install_name),
                                            offline_installer.read().decode('cp437'),
                                            authorization_string)
    ...
    def check_and_create_new_github_release():
        ...
        release.send_github_release_posts(json_post_contents, username)
    ...
    def confirm_pypi_release_state():
        ...
        release.confirm_version_changed(tag_name)
        release.confirm_release_docs_format(tag_name)
